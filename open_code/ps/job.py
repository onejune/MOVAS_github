...
class JobRunnder(object):
    ...
    def _check_python_env(self):
        if os.path.isfile(self._python_env):
            ...
            if not os.path.isdir(self._python_env + '/.pyenv'):
                message = "%r is not a valid python-env, " % self._python_env
                message += "because subdirectory .pyenv is not found in it"
                raise RuntimeError(message)
            ...
            if dir_mtime > tgz_mtime:
                args = ['tar', '-czf', tgz_path, '-C', self._python_env, '.pyenv']
    ...
    def _get_python_executable_path(self):
        if self._is_local_mode:
            ...
            python_path = './python-env/.pyenv/versions/%s/bin/python' % self._python_ver
        return python_path

    def _get_cluster_ld_library_path(self):
        ld_library_path = '.'
        ld_library_path += ':/usr/hdp/3.0.1.0-187/hadoop-hdfs/lib/native'
        ld_library_path += ':/usr/local/share/java/jre/lib/amd64/server'
        ld_library_path += ':/usr/local/share/java/jre/lib'
    ...
    def _get_spark_executors_config(self):
        ...
        if self._cmdline_args.spark_env is not None:
            for item in self._cmdline_args.spark_env:
                ...
                if name == 'OMP_NUM_THREADS':
                    omp_threads_user_config = True
        if not omp_threads_user_config:
            conf['spark.executorEnv.OMP_NUM_THREADS'] = self._get_executor_cores()
        args = []
    ...
    def _get_spark_files_config(self):
        args = []
        from ps.url_utils import use_s3a
        if not self._is_local_mode:
    ...
    def find_files(self):
        ...
        scan_files('.')
        files.append('s3://mob-emr-test/xuchen/libstdc++.so.6#libstdc++.so.6')
        files.append('s3://mob-emr-test/xuchen/libgcc_s.so.1#libgcc_s.so.1')
