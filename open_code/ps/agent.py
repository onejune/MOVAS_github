...
from ._ps import ActorMessage
from ._ps import PSRunner
from ._ps import PSDefaultAgent
...
class Agent(object):
    ...
    def _finalize(self):
        ...
        # the ``ps.Agent`` class from ``_ps.PSDefaultAgent``,
    ...
    def _get_actor_config(cls, args):
        ...
        conf.node_role = ActorRole.__members__[args['node_role']]
    ...
    @property
    def _criterion(self):
        criterion = getattr(self, '_Agent__criterion', None)
        if criterion is None:
            criterion = self._create_criterion()
            self.__criterion = criterion
        return criterion

    def update_criterion(self, predictions, labels):
        self._criterion.accumulate(predictions.data.numpy(), labels.data.numpy())

    def push_criterion(self):
        body = dict(command='PushCriterion')
        req = ActorMessage()
        req.body = json.dumps(body)
        req.receiver = 0 << 4 | 8 | 1
        states = self._criterion.get_states()
        for state in states:
            req.add_slice(state)
        def push_criterion_callback(req, res):
            self.clear_criterion()
        self.send_request(req, push_criterion_callback)

    def clear_criterion(self):
        self._criterion.clear()

    def handle_request(self, req):
        body = json.loads(req.body)
        command = body.get('command')
        if command == 'PushCriterion':
            states = ()
            for i in range(req.slice_count):
                states += req.get_slice(i),
            accum = self._criterion
            delta = ModelCriterion.from_states(states)
            accum.merge(delta)
