...
for line in fin:
    ...
    algo_price = '0'
    try:
        dtm,ad_type,platform,is_hb,ranker,package_name,bucket = arr[0].split('\t')
    ...
    key = [dtm, ad_type, platform, is_hb, ranker, tags, bucket]
    key_s = '\t'.join(key)
...
fout.write('dtm\tad_type\tplatform\tis_hb\tranker\tad_tags\tbucket\timp\tins\trev\tcost\treq\talgo_price\tcr_ivr\n')
