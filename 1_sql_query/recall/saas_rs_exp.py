...
sql = """
   SELECT exp_id, devid_type, plf, cc,bidding_type, client_type, cam_type
    , count(1) AS imp, sum(p_ivr) AS p_ins
    , sum(if(req IS NULL, 0, 1)) AS ins
    , sum(cost) AS cost, sum(rev) AS rev
    FROM (
    SELECT req, exp_id, cc, plf, bidding_type, cam_type
        , client_type, p_ivr
        ...
        END AS devid_type
    FROM (
        SELECT D.requestid AS req, exp_id, cc, plf, bidding_type, cam_type
...
GROUP BY exp_id, devid_type, plf, cc,bidding_type,client_type,cam_type
