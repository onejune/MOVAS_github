...
object RunPredictionCheck {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val days = ((sdf.parse(end_date).getTime - sdf.parse(start_date).getTime)/86400/1000 + 1).toInt
      println("total days: %d".format(days))
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[PredictionCheck]))
      ...
      val predictionCheck = new PredictionCheck()
      (0 until days).foreach{i =>
        predictionCheck.extract(sc, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
      }
