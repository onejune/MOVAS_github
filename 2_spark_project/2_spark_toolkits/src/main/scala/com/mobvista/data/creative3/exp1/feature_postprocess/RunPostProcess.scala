  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[FeaturePostProcess]))
      val spark = SparkSession
        .builder()
        .appName("bh: hercules creative3 exp1 feature post process")
        .config(conf)
        .enableHiveSupport()
        .getOrCreate()
      val sc = spark.sparkContext

      val featurePostProcess = new FeaturePostProcess()
      (0 until hours).foreach{i =>
        featurePostProcess.postProcess(sc, sdf.format(new Date(sdf.parse(start_date_hour).getTime + i * 3600 * 1000)))
      }
