...
object RunMofNginxToImpression {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[MofNginxToImpression]))
      val spark = SparkSession
        .builder()
        .appName("bh: hercules join data for more offer daily v2")
        .config(conf)
        .enableHiveSupport()
        .getOrCreate()
      val sc = spark.sparkContext

      val mofNginxToImpression = new MofNginxToImpression()
      (0 until day).foreach{i =>
        mofNginxToImpression.generate(sc, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
      }
