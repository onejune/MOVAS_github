  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[MofDataGeneration]))
      val spark = SparkSession
        .builder()
        .appName("bh: hercules join data for more offer daily")
        .config(conf)
        .enableHiveSupport()
        .getOrCreate()
      val sc = spark.sparkContext

      val mofDataGeneration = new MofDataGeneration()
      (0 until day).foreach{i =>
        mofDataGeneration.generate(sc, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
//        joinData.join(sc, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
//        mofDataNginxToImpre.getData(sc, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
      }
