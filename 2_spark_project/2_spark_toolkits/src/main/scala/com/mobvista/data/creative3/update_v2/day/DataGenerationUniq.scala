package com.mobvista.data.creative3.update_v2.day
...
//    val realtimePath = "s3://mob-emr-test/guangxue/real_time_mwho/%s".format(date)
    ...
    val outPath = "s3://mob-emr-test/baihai/m_sys_model/creative3_v2/train_data_uniq_daily/%s".format(date)
    ...
    val baseData =
      ...
      .reduceByKey({(base1: String, base2: String) => base1}, 1000)
...
//            !adType.contains("appwall") &&
      ...
      .reduceByKey({(creative1: String, creative2: String) => creative1}, 1000)
...
//      .foreach{case(key, cnt) => println("creative\t%s: %d".format(key, cnt))}
    ...
    val joinData =
      baseData
        .leftOuterJoin(creativeData, 1000)
