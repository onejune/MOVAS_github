...
object RunUserFeatureGeneration {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[UserFeatureGeneration]))
      val spark = SparkSession
        .builder()
        .appName("bh: hercules mvp_mining user feature generation")
        .config(conf)
        .enableHiveSupport()
        .getOrCreate()

      val userFeatureGeneration = new UserFeatureGeneration()
      (0 until day).foreach{i =>
        userFeatureGeneration.generate(spark, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
      }
