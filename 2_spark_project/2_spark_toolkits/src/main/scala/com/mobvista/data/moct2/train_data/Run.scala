  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[BuildTrainData]))
      val spark = SparkSession
        .builder()
        .appName("moct2.0 train data run")
        .config(conf)
        .enableHiveSupport()
        .getOrCreate()
      val sc = spark.sparkContext

      val dataGeneration = new BuildTrainData()
      (0 until day).foreach {
        i =>
        dataGeneration.perform(sc, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
      }
