  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[ExtractCpcLabel]))
      val spark = SparkSession
        .builder()
        .appName("bh: hercules creative3 exp1 extract cpc label")
        .config(conf)
        .enableHiveSupport()
        .getOrCreate()
      val sc = spark.sparkContext

      val extractCpcLabel = new ExtractCpcLabel()
      (0 until hours).foreach{i =>
        extractCpcLabel.extract(sc, sdf.format(new Date(sdf.parse(start_date_hour).getTime + i * 3600 * 1000)))
      }
