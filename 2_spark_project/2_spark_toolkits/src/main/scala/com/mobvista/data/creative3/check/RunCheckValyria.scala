  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val conf = new SparkConf()
        ...
        .registerKryoClasses(Array(classOf[CheckValyria]))
      ...
      val checkValyria = new CheckValyria()
      (0 until hours).foreach{i =>
        checkValyria.check(sc, sdf.format(new Date(sdf.parse(start_date_hour).getTime + i * 3600 * 1000)))
      }
