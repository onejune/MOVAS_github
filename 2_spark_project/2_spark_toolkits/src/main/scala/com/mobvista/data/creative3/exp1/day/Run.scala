...
import com.mobvista.data.creative3.update_v2.day.DataGenerationUniq
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val spark = SparkSession
        .builder()
        .appName("hercules creative3 appwall data test")
        .config(conf)
        .enableHiveSupport()
        .getOrCreate()
      ...
      (0 until day).foreach{i =>
        dataGeneration.generate(sc, sdf.format(new Date(sdf.parse(start_date).getTime + i * 86400 * 1000)))
      }
