...
class DataGeneration {
  ...
  def genData(sc: SparkContext, dateHour: String): RDD[String] = {
    ...
    sc.textFile(realtimePath)
      ...
      .map{case(key, (realtime, creArrOpt)) =>
        ...
        Array(realtime, creArr.mkString("\002")).mkString("\002")
      }

  }

  def genFeature(sc: SparkContext,
                 data: RDD[String],
                 columnNamePath: String,
                 combineSchemaPath: String,
                 removedFeaturePath: String,
                 dateHour: String): Unit = {

    val dataSource = new DataSource(sc, columnNamePath, combineSchemaPath, removedFeaturePath)
    val dataSourceBR = sc.broadcast(dataSource)


    val date = dateHour.substring(0, 8)
    val outPath = "s3://mob-emr-test/baihai/test/fm_train_data/%s/%s".format(date, dateHour)
    println("outPath: %s".format(outPath))

    data.repartition(1000)
      .map { line =>
        val (label, feaArr) = dataSourceBR.value.extractFeature(line)
        label + "\t" + feaArr.map{fea => fea + ":1"}.mkString("\t")
      }
  ...
  def generate(sc: SparkContext,
    ...
    val data = genData(sc, dateHour)
    genFeature(sc, data, columnNamePath, combineSchemaPath, removedFeaturePath, dateHour)
