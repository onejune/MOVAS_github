...
input_prefix="s3://mob-emr-test/lin.zhao/m_model_online/fm_raw_data"
output_prefix="s3://mob-emr-test/lin.zhao/m_model_online/fm_train_data"
...
function collect_data() {
  ...
  do
    hadoop fs -ls $input_prefix/${cur_hour:0:8}/$cur_hour
    if [ $? -eq 0 ];then
...
aws s3 cp done_time_record ${output_prefix}/done_time_record
