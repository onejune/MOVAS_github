...
flags.DEFINE_bool("gating", False, "Gating for NetVLAD")
...
class LightVLAD():
    def __init__(self, feature_size, max_frames, cluster_size, add_batch_norm,
                 is_training):
...
class NetVLAGD():
    ...
    def forward(self, reshaped_input):
        ...
        gate_weights = tf.get_variable(
            "gate_weights", [1, self.cluster_size, self.feature_size],
            initializer=tf.random_normal_initializer(
                stddev=1 / math.sqrt(self.feature_size)))
...
class GatedDBoF():
    def __init__(self, feature_size, max_frames, cluster_size, max_pool,
    ...
    def forward(self, reshaped_input):
        ...
        cluster_weights_2 = tf.get_variable(
            "cluster_weights_2", [feature_size, cluster_size],
            initializer=tf.random_normal_initializer(stddev=1 /
                                                     math.sqrt(feature_size)))
...
class DBoF():
    def __init__(self, feature_size, max_frames, cluster_size, activation,
                 add_batch_norm, is_training):
        self.feature_size = feature_size
        self.max_frames = max_frames
        self.is_training = is_training
...
class DbofModelLF(models.BaseModel):
  ...
  """
    def create_model(self,
        ...
        video_Dbof = DBoF(1024, max_frames, cluster_size, cluster_activation,
                          add_batch_norm, is_training)
        audio_Dbof = DBoF(128, max_frames, cluster_size / 8,
                          cluster_activation, add_batch_norm, is_training)
