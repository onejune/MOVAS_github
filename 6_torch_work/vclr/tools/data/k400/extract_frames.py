...
def extract_frames(source_dir, target_dir):
    ...
    for class_index in source_classes:
        ...
        for each_video in videos:
            ...
            video_prefix = each_video.rstrip(".mp4")
            ...
            try:
                ...
                subprocess.call(
                    "ffmpeg -nostats -loglevel 0 -i %s -filter:v fps=fps=30 -s 340x256 -q:v 2 %s"
                    % (source_video_name, target_frames),
                    shell=True,
                )
                ...
                if len(video_frames) == 300:
                    ...
                    for i in range(300, len(video_frames)):
                        os.remove(
                            os.path.join(target_video_frames_folder, video_frames[i])
                        )
                else:
...
if __name__ == "__main__":
    ...
    assert (
        args.target_dir
    ), "You must give the traget_dir for storing the extracted frames!"
