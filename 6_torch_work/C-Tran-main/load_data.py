...
from dataloaders.mtg_dataset import MtgDataset
import warnings
...
def get_data(args):
    ...
    if dataset == 'coco':
        ...
        train_dataset = Coco80Dataset(split='train',
                                      num_labels=args.num_labels,
                                      data_file=os.path.join(
                                          coco_root, train_data_name),
        ...
        train_dataset = Coco1000Dataset(ann_dir,
                                        data_dir,
                                        split='train',
                                        transform=trainTransform,
                                        known_labels=args.train_known_labels,
                                        testing=False)
        valid_dataset = Coco1000Dataset(ann_dir,
                                        data_dir,
                                        split='val',
                                        transform=testTransform,
                                        known_labels=args.test_known_labels,
        ...
        train_dataset = NewsDataset(ann_dir,
                                    split='train',
                                    transform=trainTransform,
                                    known_labels=0,
                                    testing=False)
        valid_dataset = NewsDataset(ann_dir,
                                    split='test',
                                    transform=testTransform,
                                    known_labels=args.test_known_labels,
                                    testing=True)
        ...
        train_dataset = Voc07Dataset(img_dir=img_dir,
                                     ...
                                     testing=False,
                                     use_difficult=False)
        valid_dataset = Voc07Dataset(img_dir=img_dir,
        ...
        img_dir = os.path.join(img_root, 'mtg_202105_06_multi_label_video')
        voc_root = './data/'
        train_label_path = os.path.join(voc_root, 'train.csv')
        val_label_path = os.path.join(voc_root, 'val.csv')
        test_label_path = os.path.join(voc_root, 'test.csv')

        train_dataset = MtgDataset(img_dir=img_dir,
                                   image_transform=trainTransform,
                                   labels_path=train_label_path,
                                   known_labels=args.train_known_labels,
                                   testing=False,
                                   use_difficult=False)
        valid_dataset = MtgDataset(img_dir=img_dir,
                                   image_transform=testTransform,
                                   labels_path=val_label_path,
                                   known_labels=args.test_known_labels,
                                   testing=True)
        test_dataset = MtgDataset(img_dir=img_dir,
                                  image_transform=testTransform,
        ...
        train_list = os.path.join(cub_root, 'class_attr_data_10',
                                  'train_valid.pkl')
        valid_list = os.path.join(cub_root, 'class_attr_data_10',
                                  'train_valid.pkl')
        test_list = os.path.join(cub_root, 'class_attr_data_10', 'test.pkl')

        train_dataset = CUBDataset(image_dir,
                                   train_list,
                                   trainTransform,
                                   known_labels=args.train_known_labels,
                                   attr_group_dict=attr_group_dict,
                                   testing=False,
                                   n_groups=n_groups)
        valid_dataset = CUBDataset(image_dir,
                                   valid_list,
                                   testTransform,
                                   known_labels=args.test_known_labels,
                                   attr_group_dict=attr_group_dict,
                                   testing=True,
                                   n_groups=n_groups)
        test_dataset = CUBDataset(image_dir,
                                  test_list,
                                  testTransform,
                                  known_labels=args.test_known_labels,
                                  attr_group_dict=attr_group_dict,
