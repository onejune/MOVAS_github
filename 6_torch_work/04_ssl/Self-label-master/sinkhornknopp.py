...
def cpu_sk(self):
    ...
    if self.hc == 1:
        self.PS = np.zeros((N, self.K),
                           dtype=self.dtype)  #最终要求的cluster分配矩阵[N, K]
    else:
    ...
    for batch_idx, (data, _, _selected) in enumerate(self.pseudo_loader):
        ...
        if self.hc == 1:
            ...
            self.PS[_selected, :] = p.detach().cpu().numpy().astype(
                self.dtype)  #初始化PS当前样本所对应行为predictor的分类结果
        else:
            p = self.model(data)
            self.PS_pre[_selected, :] = p.detach().cpu().numpy().astype(
                self.dtype)
        batch_time.update(time.time() - now)
        now = time.time()
        if batch_idx % 50 == 0:
            print(
                f"Aggregating batch {batch_idx:03}/{l_dl}, speed: {mass / batch_time.avg:04.1f}Hz",
                end='\r',
                flush=True)
    self.model.headcount = self.hc
    print("Aggreg of outputs  took {0:.2f} min".format(
        (time.time() - now) / 60.),
          flush=True)
    ...
    if self.hc == 1:
        ...
        for nh in range(self.hc):
            ...
            self.PS = (
                self.PS_pre @ tl.weight.cpu().numpy().T.astype(self.dtype) +
                tl.bias.cpu().numpy().astype(self.dtype))
            print(f"matmul took {(time.time() - time_mat)/60:.2f}min",
                  flush=True)
...
def gpu_sk(self):
    ...
    start_t = time.time()
    if self.hc == 1:
        self.PS, indices = aggreg_multi_gpu(self.model,
                                            self.pseudo_loader,
                                            hc=self.hc,
                                            dim=self.outs[0],
                                            TYPE=self.dtype)
        ...
        time.sleep(1)
        self.PS_pre, indices = aggreg_multi_gpu(self.model,
                                                self.pseudo_loader,
                                                hc=self.hc,
                                                dim=self.presize,
                                                TYPE=torch.float32)
    ...
    if self.hc == 1:
        ...
        for nh in range(self.hc):
            ...
            # apply last FC layer (a matmul and adding of bias)
            self.PS = gpu_mul_AB(self.PS_pre,
                                 tl.weight.t(),
                                 c=tl.bias,
                                 dim=self.outs[nh],
                                 TYPE=self.dtype)
            print("matmul took %smin" % ((time.time() - time_mat) / 60.),
                  flush=True)
...
def optimize_L_sk(self, nh=0):
    ...
    while err > 1e-1:
        ...
        c = c_new
        _counter += 1
    print("error: ", err, 'step ', _counter,
          flush=True)  # " nonneg: ", sum(I), flush=True)
    # inplace calculations.
    self.PS *= np.squeeze(c)
    self.PS = self.PS.T
    self.PS *= np.squeeze(r)
    self.PS = self.PS.T
    argmaxes = np.nanargmax(self.PS,
                            0)  # size N：返回指定轴上的最大值的索引,实际就是每条样本对应的类别index
    newL = torch.LongTensor(argmaxes)
    self.L[nh] = newL.to(self.dev)
    print('opt took {0:.2f}min, {1:4d}iters'.format(((time.time() - tt) / 60.),
                                                    _counter),
          flush=True)
...
def optimize_L_sk_multi(self, nh=0):
    ...
    tt = time.time()
    r = torch.ones(
        (self.outs[nh], 1), device='cuda:0', dtype=self.dtype) / self.outs[nh]
    ...
    while err > 1e-1:
        r = inv_K / (gpu_mul_xA(
            c.t(), self.PS, ngpu=ngpu, splits=splits,
            TYPE=self.dtype)).t()  # ((1xN)@(NxK)).T = Kx1
        c_new = inv_N / (gpu_mul_Ax(
            self.PS, r, ngpu=ngpu, splits=splits, TYPE=self.dtype)
                         )  # (NxK)@(K,1) = N x 1
        torch.cuda.synchronize()  # just in case
        if _counter % 10 == 0:
            err = torch.sum(torch.abs((c.squeeze() / c_new.squeeze()) -
                                      ones)).cpu().item()
        c = c_new
        _counter += 1
    ...
    for i, qq in enumerate(self.PS):
        torch.mul(qq,
                  c[splits[i]:splits[i + 1], :].to('cuda:' + str(i + 1)),
                  out=qq)
    [
        torch.mul(r.to('cuda:' + str(i + 1)).t(), qq, out=qq)
        for i, qq in enumerate(self.PS)
    ]
