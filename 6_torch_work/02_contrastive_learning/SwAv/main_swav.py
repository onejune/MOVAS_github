...
parser.add_argument("--epoch_queue_starts",
                    type=int,
                    default=15,
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue):
    ...
    for it, inputs in enumerate(train_loader):
        ...
        for i, crop_id in enumerate(args.crops_for_assign):
            ...
            subloss = 0
            for v in np.delete(
                    np.arange(np.sum(args.nmb_crops)),
                    crop_id):  #avoid duplicate crop_id in figuring loss
                x = output[bs * v:bs * (v + 1)] / args.temperature
                subloss -= torch.mean(
                    torch.sum(q * F.log_softmax(x, dim=1),
                              dim=1))  #cross_entropy loss
