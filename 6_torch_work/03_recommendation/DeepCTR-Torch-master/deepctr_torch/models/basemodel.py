    ...
    def forward(self, X, sparse_feat_refine_weight=None):
        ...
        sparse_embedding_list = [
            self.embedding_dict[feat.embedding_name](
                X[:, self.feature_index[feat.name][0]:self.
                  feature_index[feat.name][1]].long())
            for feat in self.sparse_feature_columns
        ]

        dense_value_list = [
            X[:, self.feature_index[feat.name][0]:self.feature_index[feat.
                                                                     name][1]]
            for feat in self.dense_feature_columns
        ]

        varlen_embedding_list = get_varlen_pooling_list(
            self.embedding_dict, X, self.feature_index,
            self.varlen_sparse_feature_columns, self.device)

        sparse_embedding_list += varlen_embedding_list

        linear_logit = torch.zeros([X.shape[0],
                                    1]).to(sparse_embedding_list[0].device)
        if len(sparse_embedding_list) > 0:
            sparse_embedding_cat = torch.cat(sparse_embedding_list, dim=-1)
            if sparse_feat_refine_weight is not None:
                # w_{x,i}=m_{x,i} * w_i (in IFM and DIFM)
                sparse_embedding_cat = sparse_embedding_cat * sparse_feat_refine_weight.unsqueeze(
                    1)
            sparse_feat_logit = torch.sum(sparse_embedding_cat,
                                          dim=-1,
                                          keepdim=False)
            linear_logit += sparse_feat_logit
        if len(dense_value_list) > 0:
            dense_value_logit = torch.cat(dense_value_list,
                                          dim=-1).matmul(self.weight)
...
class BaseModel(nn.Module):
    def __init__(self,
        ...
        self.linear_model = Linear(linear_feature_columns,
                                   self.feature_index,
                                   device=device)
    ...
    def fit(self,
        ...
        #context indicator, indicate the same request or user
        ...
        if validation_data:
            ...
            if context_indicator.any():
                indicator_train, indicator_val = context_indicator[0 : split_at], context_indicator[split_at : -1]
        ...
        # 这里会把一条样本的特征都concat成一个一维数组
        concat_x = np.concatenate(x, axis=-1)
        ten_x = torch.from_numpy(concat_x)
        ten_y = torch.from_numpy(y)
        
        ten_indicator = []
        if indicator_train.any():
            ten_indicator = torch.from_numpy(indicator_train)
        else:
            ten_indicator = ten_y
        #构造出一条条样本对
        train_tensor_data = Data.TensorDataset(ten_x, ten_y, ten_indicator)
  
        if batch_size is None:
            batch_size = 256

        #将本层及子层的training属性同时设为true,对于一些含有BatchNorm，Dropout等层的模型，在训练时使用的forward和验证时使用的forward在计算上不太一样。在前向训练的过程中指定当前模型是训练还是在验证。
        model = self.train()
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    for _, (x_train, y_train, indicator) in t:
                        x = x_train.to(self.device).float()
                        y = y_train.to(self.device).float()
                        #pos_num = torch.sum(y, dim=0).item()
                        #if pos_num == 0:
                        #    continue
                        #print('indicator in batch:\n', indicator)
                        #print('sample:\n', x)
                        y_pred = model(x).squeeze()

                        optim.zero_grad()
                        loss = loss_func(y_pred, y.squeeze(), context_indicator = indicator, reduction = 'sum')
                        ...
                        #    MovasLogger.add_log(content = '-->name:%s -->grad_requirs:%s -->grad_value:%s' % (name, parms.requires_grad,parms.grad))
    ...
    def predict(self, x, batch_size=256):
        ...
        test_loader = DataLoader(dataset=tensor_data,
                                 shuffle=False,
                                 batch_size=batch_size)
    ...
    def input_from_feature_columns(self,
                                   X,
                                   feature_columns,
                                   embedding_dict,
                                   support_dense=True):
    ...
    def compute_input_dim(self,
                          ...
                          feature_group=False):
        sparse_feature_columns = list(
            filter(lambda x: isinstance(x, (SparseFeat, VarLenSparseFeat)),
                   feature_columns)) if len(feature_columns) else []
        dense_feature_columns = list(
            filter(lambda x: isinstance(x, DenseFeat),
                   feature_columns)) if len(feature_columns) else []

        dense_input_dim = sum(map(lambda x: x.dimension,
                                  dense_feature_columns))
        if feature_group:
            sparse_input_dim = len(sparse_feature_columns)
        else:
            sparse_input_dim = sum(feat.embedding_dim
                                   for feat in sparse_feature_columns)
        input_dim = 0
        if include_sparse:
