...
class DeepFM(BaseModel):
    ...
    def forward(self, X):
        '''
        1. 对输入feature做embedding转换,即query embedding_dict;
        2. 这里FM模块和Deep模块是共享embedding(sparse_embedding_list);
        3. 在init的时候会调用create_embedding_matrix创建embedding dict；
        4. 每个特征的embedding size在构造SparseFeat对象的时候设置；
        '''
        sparse_embedding_list, dense_value_list = self.input_from_feature_columns(
            X, self.dnn_feature_columns, self.embedding_dict)
        #这里是为了计算1阶的sum weight，FM部分只计算了2阶
        logit = self.linear_model(X)

        if self.use_fm and len(sparse_embedding_list) > 0:
            fm_input = torch.cat(sparse_embedding_list, dim=1)
            logit += self.fm(fm_input)
        #dense feature不需要embedding，本身就是个归一化的数值特征，直接concat到sparse embedding后面进入DNN，想想pretrained embedding
        if self.use_dnn:
