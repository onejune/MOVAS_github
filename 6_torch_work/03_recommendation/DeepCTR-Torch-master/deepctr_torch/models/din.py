...
class DIN(BaseModel):
    ...
    def __init__(self,
        ...
        print('-' * 20, 'din-init', '-' * 20)
        print('varlen_sparse_feature_columns:',
              self.varlen_sparse_feature_columns)
        print('history_fc_names:', self.history_fc_names)
    ...
    def forward(self, X):
        ...
        keys_emb_list = embedding_lookup(
            ...
            to_list=True)
        dnn_input_emb_list = embedding_lookup(X,
                                              self.embedding_dict,
                                              self.feature_index,
                                              self.sparse_feature_columns,
                                              to_list=True)
        ...
        # 把每条样本的多个hist_feature的embedding按history id concat到一起
        query_emb = torch.cat(query_emb_list, dim=-1)  # [B, 1, E]
        keys_emb = torch.cat(keys_emb_list, dim=-1)  # [B, T, E]

        keys_length_feature_name = [
            feat.length_name for feat in self.varlen_sparse_feature_columns
            if feat.length_name is not None
        ]

        # print('keys_emb_list:', len(keys_emb_list), keys_emb_list)
        # print('keys_emb:', len(keys_emb), keys_emb)
        # print('keys_length_feature_name:', keys_length_feature_name)
