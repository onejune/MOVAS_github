...
class FM(nn.Module):
    ...
    def forward(self, inputs):
        '''
        1. inputs是个(batch_size,field_size,embedding_size)的矩阵，embedding size就是每个隐向量的长度，默认是4
        2. 这里只计算了FM的second order部分
        '''
        fm_input = inputs
...
class AFMLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.projection_h = nn.Parameter(torch.Tensor(self.attention_factor,
                                                      1))
    ...
    def forward(self, inputs):
        ...
        attention_temp = F.relu(
            torch.tensordot(bi_interaction, self.attention_W, dims=([-1],
                                                                    [0])) +
            self.attention_b)
        ...
        afm_out = torch.tensordot(attention_output,
                                  self.projection_p,
                                  dims=([-1], [0]))
        return afm_out
...
class InteractingLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        querys = torch.stack(
            torch.split(querys, self.att_embedding_size, dim=2))
        keys = torch.stack(torch.split(keys, self.att_embedding_size, dim=2))
        values = torch.stack(
            torch.split(values, self.att_embedding_size, dim=2))
        inner_product = torch.einsum('bnik,bnjk->bnij', querys,
                                     keys)  # head_num None F F
        if self.scaling:
            inner_product /= self.att_embedding_size**0.5
        self.normalized_att_scores = F.softmax(inner_product,
                                               dim=-1)  # head_num None F F
        result = torch.matmul(self.normalized_att_scores,
                              values)  # head_num None F D

        result = torch.cat(torch.split(
            result,
            1,
        ), dim=-1)
    ...
    def forward(self, inputs):
        ...
        for i in range(self.layer_num):
            if self.parameterization == 'vector':
                ...
                xl_w = torch.matmul(self.kernels[i],
                                    x_l)  # W * xi  (bs, in_features, 1)
    ...
    def forward(self, inputs):
        ...
        for i in range(self.layer_num):
            ...
            for expert_id in range(self.num_experts):
                ...
                v_x = torch.matmul(self.V_list[i][expert_id].t(),
                                   x_l)  # (bs, low_rank, 1)
                ...
                uv_x = torch.matmul(self.U_list[i][expert_id],
            ...
            output_of_experts = torch.stack(
                output_of_experts, 2)  # (bs, in_features, num_experts)
            gating_score_of_experts = torch.stack(gating_score_of_experts,
                                                  1)  # (bs, num_experts, 1)
            moe_out = torch.matmul(output_of_experts,
                                   gating_score_of_experts.softmax(1))
    ...
    def forward(self, inputs):
        ...
        for i in range(num_inputs - 1):
            for j in range(i + 1, num_inputs):
                row.append(i)
                col.append(j)
        p = torch.cat([embed_list[idx] for idx in row],
                      dim=1)  # batch num_pairs k
    ...
    def __init__(self,
        ...
        if self.kernel_type == 'mat':
            ...
            self.kernel = nn.Parameter(
                torch.Tensor(embed_size, num_pairs, embed_size))
...
class ConvLayer(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(1, l + 1):
            ...
            module_list.append(
                KMaxPooling(k=min(k, filed_shape), axis=2,
