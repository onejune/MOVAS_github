...
class SequencePoolingLayer(nn.Module):
    ...
    def forward(self, seq_value_len_list):
        if self.supports_masking:
            ...
            mask = self._sequence_mask(user_behavior_length,
        ...
        mask = torch.repeat_interleave(mask, embedding_size,
                                       dim=2)  # [B, maxlen, E]
        ...
        if self.mode == 'mean':
            ...
            hist = torch.div(
                hist,
                user_behavior_length.type(torch.float32) + self.eps)
...
class AttentionSequencePoolingLayer(nn.Module):
    ...
    def forward(self, query, keys, keys_length, mask=None):
        ...
        Input shape
          ...
          - keys is a 3D tensor with shape:   ``(batch_size, T, embedding_size), T is the length of history``
        ...
        Output shape
          - 3D tensor with shape: ``(batch_size, 1, embedding_size)``.
        """
        # max_length是所有样本history list的最大长度，会用0进行padding，以保证所有样本特征对齐，
        # keys_length是每个样本history list的实际长度，每个样本不同
        ...
        if self.supports_masking:
            ...
            keys_masks = torch.arange(max_length,
                                      device=keys_length.device,
                                      dtype=keys_length.dtype).repeat(
                                          batch_size, 1)  # [B, T]
            # print('keys_masks1:', keys_masks.size(), keys_masks)
            keys_masks = keys_masks < keys_length.view(-1, 1)  # 0, 1 mask
            # print('keys_masks2:', keys_masks.size(), keys_masks)
            keys_masks = keys_masks.unsqueeze(1)  # [B, 1, T]
            # print('keys_masks3:', keys_masks.size(), keys_masks)
...
class DynamicGRU(nn.Module):
    ...
    def forward(self, input, att_scores=None, hx=None):
        ...
        if hx is None:
            hx = torch.zeros(max_batch_size,
                             self.hidden_size,
                             dtype=input.dtype,
                             device=input.device)

        outputs = torch.zeros(input.size(0),
                              self.hidden_size,
                              dtype=input.dtype,
                              device=input.device)
