...
def combined_dnn_input(sparse_embedding_list, dense_value_list):
    if len(sparse_embedding_list) > 0 and len(dense_value_list) > 0:
        sparse_dnn_input = torch.flatten(torch.cat(sparse_embedding_list,
                                                   dim=-1),
                                         start_dim=1)
        dense_dnn_input = torch.flatten(torch.cat(dense_value_list, dim=-1),
                                        start_dim=1)
        return concat_fun([sparse_dnn_input, dense_dnn_input])
    elif len(sparse_embedding_list) > 0:
        return torch.flatten(torch.cat(sparse_embedding_list, dim=-1),
                             start_dim=1)
...
def get_varlen_pooling_list(embedding_dict, features, feature_index,
    ...
    # print('-' * 20, 'get_varlen_pooling_list', '-' * 20)
    # print('embedding_dict:', embedding_dict)

    for feat in varlen_sparse_feature_columns:
        fea_name = feat.name
        fea_idx_s = feature_index[feat.name][0]
        fea_idx_e = feature_index[feat.name][1]
        emb_name = feat.embedding_name
        input_data = features[:, fea_idx_s:fea_idx_e].long()

        # print(fea_name, emb_name, fea_idx_s, fea_idx_e)

        seq_emb = embedding_dict[emb_name](input_data)
        if feat.length_name is None:
            seq_mask = features[:, feature_index[feat.name][0]:feature_index[
                feat.name][1]].long() != 0

            emb = SequencePoolingLayer(mode=feat.combiner,
                                       supports_masking=True,
                                       device=device)([seq_emb, seq_mask])
        else:
            seq_length = features[:, feature_index[feat.length_name][0]:
                                  feature_index[feat.length_name][1]].long()
            emb = SequencePoolingLayer(mode=feat.combiner,
                                       supports_masking=False,
                                       device=device)([seq_emb, seq_length])
        varlen_sparse_embedding_list.append(emb)
    return varlen_sparse_embedding_list


def create_embedding_matrix(feature_columns,
                            init_std=0.0001,
                            linear=False,
                            sparse=False,
                            device='cpu'):
    ...
    embedding_dict = nn.ModuleDict({
        feat.embedding_name:
        nn.Embedding(feat.vocabulary_size,
...
def varlen_embedding_lookup(X, embedding_dict, sequence_input_dict,
    ...
    # print('-' * 20, 'varlen_embedding_lookup', '-' * 20)
    ...
    for fc in varlen_sparse_feature_columns:
        ...
        if fc.use_hash:
            ...
            lookup_idx = sequence_input_dict[feature_name]
        varlen_embedding_vec_dict[feature_name] = embedding_dict[
            embedding_name](
